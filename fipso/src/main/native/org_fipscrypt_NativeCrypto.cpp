...
/**
 * Native glue for Java class org.fipscrypt.NativeCrypto
 */
 ...
 */
static jlong NativeCrypto_EVP_DigestInit(JNIEnv* env, jclass, jlong evpMdRef, jint evp_flags) {
    ...
    EVP_MD_CTX_set_flags(ctx.get(), evp_flags);

    int ok = EVP_DigestInit_ex(ctx.get(), evp_md, NULL);
    if (ok == 0) {
...
static JNINativeMethod sNativeCryptoMethods[] = {
    ...
    NATIVE_METHOD(NativeCrypto, EVP_DigestInit, "(JI)J"),
    ...
    NATIVE_METHOD(NativeCrypto, create_BIO_InputStream, ("(L" TO_STRING(JNI_JARJAR_PREFIX) "org/fipscrypt/OpenSSLBIOInputStream;)J")),
...
static void initialize_fipscrypt(JNIEnv* env) {
    jniRegisterNativeMethods(env, TO_STRING(JNI_JARJAR_PREFIX) "org/fipscrypt/NativeCrypto",
                             sNativeCryptoMethods, NELEM(sNativeCryptoMethods));

    ScopedLocalRef<jclass> localClass(env,
            env->FindClass(TO_STRING(JNI_JARJAR_PREFIX) "org/fipscrypt/OpenSSLBIOInputStream"));
