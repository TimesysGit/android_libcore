...
public class OpenSSLMessageDigestJDK extends MessageDigest implements Cloneable {
    ...
    /**
     * Holds the flags to be passed when initializing the context
     */
    private final int evp_flags;
    ...
    private static final int EVP_MD_CTX_FLAG_NON_FIPS_ALLOW = 0x0008;

    /**
     * Creates a new OpenSSLMessageDigest instance for the given algorithm
     * name.
     */
    private OpenSSLMessageDigestJDK(String algorithm, long evp_md, int evp_flags, int size)
            throws NoSuchAlgorithmException {
        super(algorithm);
        this.evp_md = evp_md;
        this.evp_flags = evp_flags;
        this.size = size;
    }
    ...
    private long getCtx() {
        if (ctx == 0) {
            ctx = NativeCrypto.EVP_DigestInit(evp_md, evp_flags);
        }
        return ctx;
    }
    ...
    public static class MD5 extends OpenSSLMessageDigestJDK {
        ...
        public MD5() throws NoSuchAlgorithmException {
            super("MD5",EVP_MD, 0, SIZE);
        }
    }
    
    public static class MD5_FIPS extends OpenSSLMessageDigestJDK {
        private static final long EVP_MD = NativeCrypto.EVP_get_digestbyname("md5");
        private static final int SIZE = NativeCrypto.EVP_MD_size(EVP_MD);
        public MD5_FIPS() throws NoSuchAlgorithmException {
            super("MD5_FIPS",EVP_MD, EVP_MD_CTX_FLAG_NON_FIPS_ALLOW, SIZE);
        }
    }
